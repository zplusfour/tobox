...
router.get('/del/:id', async (req: express.Request, res: express.Response) => {
	const { id } = req.params;
	const user = await User.findOne({ username: cryptr.decrypt(req.cookies['user']) });
	if (user) {
...
router.post('/edit/:id', async (req: express.Request, res: express.Response) => {
	const { id } = req.params;
	const { title, content } = req.body;
	const user = await User.findOne({ username: cryptr.decrypt(req.cookies['user']) });
	if (user) {
		...
		todo.date = new Date().toLocaleString() + ' (edited)';
		await user.save();
		res.redirect('/');
	} else {
		res.redirect('/signin');
